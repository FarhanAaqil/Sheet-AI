...
Your JSON Action:"""
    ...
    def run(self, query: str):
        ...
        context = "\n".join([
            ...
            for doc in self.rag.search(query)
        ])
        ...
        for i in range(5):
            ...
            try:
                ...
                response = getattr(gemini_response, "text", None)
                ...
                try:
                    ...
                    thought_process.append(
                        "⚠️ **Self-Correction:** Invalid JSON detected. Attempting repair."
                    )
                ...
                thought_process.extend([
                    ...
                    f"🛠️ **Action:** `{action}` with input `{action_input}`"
                ])
                ...
                if not tool_func:
                    ...
                    try:
                        ...
                        observation = f"Tool execution failed: {tool_error}"
    
                # -----------------------------
                # Store observation
                # -----------------------------
                obs_str = str(observation)[:1500]
                ...
                return (
                    "The agent encountered an internal error while processing your request.",
                    "\n\n".join(thought_process)
                )
        ...
        # -------------------------------------------------
        thought_process.append(
            "⚠️ Maximum reasoning steps reached. Returning fallback answer."
        )
    
        fallback_answer = (
            "I could not fully complete the request, "
            "but I gathered some useful context."
        )
